...
FaceDetectResult runFacedetect(vart::Runner* runner, const Mat& frame) {
    ...
    auto fd_post_FilterBox_start = std::chrono::high_resolution_clock::now();
    ...
    auto fd_post_FilterBox_end = std::chrono::high_resolution_clock::now();
    auto fd_post_FilterBox_duration = std::chrono::duration_cast<std::chrono::milliseconds>(fd_post_FilterBox_end - fd_post_FilterBox_start);
    std::cout << "fd_post_FilterBox time: " << fd_post_FilterBox_duration.count() << " ms" << std::endl;
    ...
    auto fd_post_NMS_start = std::chrono::high_resolution_clock::now();
    vector<size_t> res_k;
    int max_faces = 1;
    applyNMS(boxes, scores, nms_threshold, det_threshold, res_k, max_faces);
    auto fd_post_NMS_end = std::chrono::high_resolution_clock::now();
    auto fd_post_NMS_duration = std::chrono::duration_cast<std::chrono::milliseconds>(fd_post_NMS_end - fd_post_NMS_start);
    std::cout << "fd_post_NMS time: " << fd_post_NMS_duration.count() << " ms" << std::endl;
